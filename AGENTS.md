...
- **Supabase env vars**: The app requires `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` in `.env.local`. Without real Supabase credentials, the frontend renders fully but RSVP and Wishes API calls (`/api/rsvp`, `/api/wishes`) will fail at runtime. The `/api/music` endpoint works without Supabase.
